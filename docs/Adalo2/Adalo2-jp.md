...
- 「Aが決まればBが1つに決まる」という関係が成り立つ時、AをCollectionに、BをそのCollectionのPropertyにする場合が多いです。
  - ユーザーが決まれば、ユーザーのEmail、パスワード、FullNameがそれぞれ1つに決まる
  - ペットが決まれば、ペットの名前、写真、誕生日がそれぞれ1つに決まる
  - ペットの体重とその登録日時は、1匹のペットに対して複数登録されるので、ペットが決まっても1つに決まらない
  (そのため、今回はCollectionを分けた)
...
---
#### データの更新(UPDATE)
TODO: 登録したデータを更新する画面を作ろう(Formコンポーネントで自動生成)
